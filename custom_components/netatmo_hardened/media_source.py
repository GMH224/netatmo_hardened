"""Netatmo Media Source Implementation."""

from __future__ import annotations

import datetime as dt
import logging
import re
from typing import override

from homeassistant.components.media_player import BrowseError, MediaClass, MediaType
from homeassistant.components.media_source import (
    BrowseMediaSource,
    MediaSource,
    MediaSourceError,
    MediaSourceItem,
    PlayMedia,
    Unresolvable,
)
from homeassistant.config_entries import ConfigEntryState
from homeassistant.core import HomeAssistant, callback

from .const import DOMAIN, MANUFACTURER
from .coordinator import NetatmoConfigEntry, NetatmoDataHandler, async_get_loaded_entry

_LOGGER = logging.getLogger(__name__)
MIME_TYPE = "application/x-mpegURL"


class IncompatibleMediaSource(MediaSourceError):
    """Incompatible media source attributes."""


async def async_get_media_source(hass: HomeAssistant) -> NetatmoSource:
    """Set up Netatmo media source."""
    return NetatmoSource(hass, async_get_loaded_entry(hass))


class NetatmoSource(MediaSource):
    """Provide Netatmo camera recordings as media sources."""

    name: str = MANUFACTURER

    def __init__(self, hass: HomeAssistant, entry: NetatmoConfigEntry | None) -> None:
        """Initialize Netatmo source."""
        super().__init__(DOMAIN)
        self.hass = hass
        self.entry = entry

    @property
    def _data_handler(self) -> NetatmoDataHandler | None:
        """Return the data handler of the config entry, if it is loaded."""
        if self.entry is None or self.entry.state is not ConfigEntryState.LOADED:
            return None
        return self.entry.runtime_data

    @property
    def events(self) -> dict[str, dict]:
        """Return the camera events."""
        data_handler = self._data_handler
        return data_handler.events if data_handler else {}

    @override
    async def async_resolve_media(self, item: MediaSourceItem) -> PlayMedia:
        """Resolve media to a url.

        [hardened-fork] Every lookup layer is checked and every failure becomes
        ``Unresolvable``. Upstream chained ``self.events[camera_id][event_id]
        ["media_url"]`` straight after a parse that could raise ``ValueError``,
        so an identifier for an event that had since expired produced a raw
        traceback instead of "this recording is no longer available"
        (defect C-20, external findings NET-014 / SEC-008).
        """
        _, camera_id, event_id = async_parse_identifier(item)

        if not camera_id or event_id is None:
            raise Unresolvable("Incomplete Netatmo media identifier.")

        camera_events = self.events.get(camera_id)
        if camera_events is None:
            raise Unresolvable("Camera does not exist.")

        event = camera_events.get(event_id)
        if event is None:
            raise Unresolvable("Event no longer available.")

        url = event.get("media_url")
        if not url:
            raise Unresolvable("Event has no playable media.")

        return PlayMedia(url, MIME_TYPE)

    @override
    async def async_browse_media(self, item: MediaSourceItem) -> BrowseMediaSource:
        """Return media."""
        try:
            source, camera_id, event_id = async_parse_identifier(item)
        except Unresolvable as err:
            raise BrowseError(str(err)) from err

        return self._browse_media(source, camera_id, event_id)

    def _browse_media(
        self, source: str, camera_id: str, event_id: str | None
    ) -> BrowseMediaSource:
        """Browse media."""
        if camera_id and camera_id not in self.events:
            raise BrowseError("Camera does not exist.")

        if event_id is not None and event_id not in self.events.get(camera_id, {}):
            raise BrowseError("Event does not exist.")

        return self._build_item_response(source, camera_id, event_id)

    def _build_item_response(
        self, source: str, camera_id: str, event_id: str | None = None
    ) -> BrowseMediaSource:
        if event_id is not None and event_id in self.events.get(camera_id, {}):
            event = self.events[camera_id][event_id]
            created = dt.datetime.fromtimestamp(event.get("event_time", 0))
            snapshot = event.get("snapshot") or {}
            thumbnail = snapshot.get("url") if isinstance(snapshot, dict) else None
            message = remove_html_tags(event.get("message") or "")
            title = f"{created} - {message}"
        else:
            data_handler = self._data_handler
            title = (
                data_handler.cameras.get(camera_id, MANUFACTURER)
                if data_handler
                else MANUFACTURER
            )
            thumbnail = None

        if event_id is not None:
            path = f"{source}/{camera_id}/{event_id}"
        else:
            path = f"{source}/{camera_id}"

        media_class = MediaClass.DIRECTORY if event_id is None else MediaClass.VIDEO

        media = BrowseMediaSource(
            domain=DOMAIN,
            identifier=path,
            media_class=media_class,
            media_content_type=MediaType.VIDEO,
            title=title,
            can_play=bool(
                event_id is not None
                and self.events.get(camera_id, {}).get(event_id, {}).get("media_url")
            ),
            can_expand=event_id is None,
            thumbnail=thumbnail,
        )

        if not media.can_play and not media.can_expand:
            _LOGGER.debug(
                "Camera %s with event %s without media url found", camera_id, event_id
            )
            raise IncompatibleMediaSource

        if not media.can_expand:
            return media

        media.children = []
        # Append first level children
        if not camera_id:
            for cid in self.events:
                child = self._build_item_response(source, cid)
                if child:
                    media.children.append(child)
        else:
            for eid in self.events.get(camera_id, {}):
                try:
                    child = self._build_item_response(source, camera_id, eid)
                except IncompatibleMediaSource:
                    continue
                if child:
                    media.children.append(child)

        return media


def remove_html_tags(text: str) -> str:
    """Remove html tags from string."""
    clean = re.compile("<.*?>")
    return re.sub(clean, "", text)


@callback
def async_parse_identifier(
    item: MediaSourceItem,
) -> tuple[str, str, str | None]:
    """Parse a media source identifier.

    [hardened-fork] Event ids are carried as strings end to end. Upstream
    coerced them with ``int()``, which raised ``ValueError`` on any identifier
    that was not numeric - and since events are now keyed by their Netatmo
    event id rather than a timestamp, they are not numeric (defects C-19, C-20).
    """
    if not item.identifier or "/" not in item.identifier:
        return "events", "", None

    source, path = item.identifier.lstrip("/").split("/", 1)

    if source != "events":
        raise Unresolvable("Unknown source directory.")

    if "/" in path:
        camera_id, event_id = path.split("/", 1)
        if not event_id:
            raise Unresolvable("Empty event identifier.")
        return source, camera_id, event_id

    return source, path, None
