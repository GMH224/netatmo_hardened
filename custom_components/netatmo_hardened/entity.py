...
class NetatmoDeviceEntity(NetatmoBaseEntity):
    ...
    def device_description(self) -> tuple[str, str]:
        """Return the manufacturer and model of this device.

        [hardened-fork] Uses ``.get()`` with a fallback. Netatmo and pyatmo add
        device types between releases, and a direct lookup turned an unknown
        type into a ``KeyError`` during entity construction - taking down the
        whole platform rather than showing one device generically. ``device.py``
        already used the tolerant form on the same map; this makes the entity
        layer consistent with it (defect C-17).
        """
        return DEVICE_DESCRIPTION_MAP.get(
            self.device_type, (MANUFACTURER, str(self.device_type))
        )
...
def room_device_info(room: Room, via_device_id: str) -> DeviceInfo:
    ...
    """
    # [hardened-fork] No assertion, no direct lookup (defects C-16, C-17).
    manufacturer, model = DEVICE_DESCRIPTION_MAP.get(
        room.climate_type, (MANUFACTURER, "Room")
    )
    return DeviceInfo(
...
class NetatmoWeatherModuleEntity(NetatmoModuleEntity):
    ...
    def __init__(self, device: NetatmoDevice, **kwargs: Any) -> None:
        ...
        # [hardened-fork] A module whose category pyatmo could not resolve used
        # to raise AssertionError here (defect C-16).
        if self.device.device_category is None:
            raise HomeAssistantError(
                f"Netatmo module {self.device.entity_id} reports no device "
                "category and cannot be set up as a weather entity"
            )
