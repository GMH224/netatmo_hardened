...
class NetatmoCameraLight(NetatmoModuleEntity, LightEntity):
    ...
    def handle_event(self, event: dict) -> None:
        ...
        [hardened-fork] Only ``camera_id`` was guarded upstream; ``home_id``,
        ``event_type`` and ``sub_type`` were indexed directly, so a payload
        carrying a camera id but omitting any of the other three raised
        ``KeyError`` inside a dispatcher callback (defect C-9).
        """
        ...
        if (
            ...
            and data.get(ATTR_EVENT_TYPE) == EVENT_TYPE_LIGHT_MODE
        ):
            self._attr_is_on = bool(data.get("sub_type") == "on")
